// ---------------------------------------------------------------------------
// mcp.ts -- MCP server factory for Lucid Predict
// ---------------------------------------------------------------------------
...
function paramDefToZod(def: ToolParamDef): z.ZodTypeAny {
  switch (def.type) {
    ...
    case 'object':
      ...
      return z.record(z.unknown()).describe(def.description ?? '');
    default: // 'string'
      return z.string().describe(def.description ?? '');
  }
}
...
/**
 * Create and configure the Lucid Predict MCP server.
 *
 * 1. Load config from env
 * 2. Create platform registry and register adapters
...
export function createPredictServer(
  ...
  // Always register Polymarket and Manifold (public APIs)
  ...
  // Register Kalshi if credentials are available
  ...
  log.info(`${PLUGIN_NAME} MCP server created (${tools.length} tools, ${registry.size} platforms)`);
  return server;
}
