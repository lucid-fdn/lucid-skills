...
describe('loadConfig', () => {
  ...
  it('applies defaults', () => {
    ...
    expect(config.maxContractSize).toBe(50000);
    expect(config.scanSchedule).toBe('0 */6 * * *');
  });

  it('throws without supabaseUrl', () => {
    expect(() => {
      const origUrl = process.env.AUDIT_SUPABASE_URL;
      const origKey = process.env.AUDIT_SUPABASE_KEY;
      delete process.env.AUDIT_SUPABASE_URL;
      delete process.env.AUDIT_SUPABASE_KEY;
      try {
        loadConfig({} as any);
      } finally {
        process.env.AUDIT_SUPABASE_URL = origUrl;
        process.env.AUDIT_SUPABASE_KEY = origKey;
      }
  ...
  it('includes optional fields when provided', () => {
    const config = loadConfig({
      ...
      supabaseKey: 'key',
      etherscanApiKey: 'eth-key',
      slackWebhookUrl: 'https://hooks.slack.com/test',
    });
    expect(config.etherscanApiKey).toBe('eth-key');
